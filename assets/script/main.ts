...
export class main extends Component {
  ...
  start() {
    ...
    localConfig.instance.loadConfig(() => {
      ...
      // this.loadMainScene();
      this.loadLoginPortkeyScene();
    });

    this.curProgress += 5;
    if (this.loadingUI) {
      this.loadingUI.updateProgress(
        this.curProgress,
        i18n.t("main.dataLoadOver")
      );
    }
    ...
    this.loadingUI.updateProgress(
      this.curProgress,
      i18n.t("main.gameResourceLoading")
    );
  ...
  loadGameSubPackage(cb?: Function) {
    this.loadingUI.updateProgress(
      this.curProgress,
      i18n.t("main.audioResourceLoading")
    );
    assetManager.loadBundle("resources", (err) => {
      ...
      assetManager.loadBundle("textures", (err) => {
        ...
        this.loadingUI.updateProgress(
          this.curProgress,
          i18n.t("main.mappingResourceLoading")
        );
        if (err) {
          this.showSubPackageError();
          return console.error(err);
        ...
        assetManager.loadBundle("model", (err) => {
          ...
          this.loadingUI.updateProgress(
            this.curProgress,
            i18n.t("main.modelResourceLoading")
          );
          if (err) {
            this.showSubPackageError();
            return console.error(err);
  ...
  loadMainScene() {
    if (
      !this.isConfigLoaded ||
      !this.isLoginFinished ||
      !this.isSubPackageFinished
    ) {
  ...
  loadLoginPortkeyScene() {
    ...
    director.preloadScene("login-portkey", (err) => {
      this.curProgress += 5;
      this.loadingUI.updateProgress(this.curProgress, i18n.t("main.entering"));
      if (!err) {
        director.loadScene("login-portkey", function () {});
      }
    });
  }
}
